...
from blackmores.models import Product, Image
from blackmores.sniffer.models import CrawlFrontier


class blackmores(object):
    ...
    def sniff(self):
        while True:
            try:
                frontier = \
                CrawlFrontier.objects.filter(sniff_session=self.uuid, is_visited=False).order_by('created_at')[0]
                self.crawl(frontier)
            except:
                break
    ...
    def howl(self, product):
        ...
        if r.status_code == 200:
            try:
                ...
                price = float(
                    html.split('<meta itemprop="price" content="')[1].split('"')[0].partition(' ')[0].replace(',', '.'))
                if price:
                    product.update_price(price)
                    product.status = Product.OK
                else:
                    logging.error(
                        u'Could not parse price for product {0} at {1}'.format(product.code, product.get_url()))

                try:
                    product.manufacturer = \
                    html.split('itemtype="http://schema.org/Organization"')[1].split('<td itemprop="name"><strong>')[
                        1].partition('</strong>')[0]
                except:
                    logging.error(
                        u'Could not parse manufacturer for product {0} at {1}'.format(product.code, product.get_url()))

                try:
                    product.manufacturer_code = \
                    html.split('itemtype="http://schema.org/Organization"')[1].split('<td itemprop="model"><strong>')[
                        1].partition('</strong>')[0]
                except:
                    logging.error(u'Could not parse manufacturer code for product {0} at {1}'.format(product.code,
                                                                                                     product.get_url()))
                ...
                try:
                    ...
                    logging.error(
                        u'Could not parse images for product {0} at {1}'.format(product.code, product.get_url()))

                product.url = r.url
            except Exception as e:
                product.status = Product.ERROR
                logging.error(
                    u'Could not parse product {0} at {1}. Exception {2}'.format(product.code, product.get_url(),
                                                                                e.message))
